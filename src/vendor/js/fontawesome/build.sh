#!/usr/bin/env bash
# Generate the Shape tool's "shapes" custom-shape library from the
...
#   ../../../resources/libraries/shapes.csh
#
...
cd "$(dirname "$0")"
SUB="../../fontawesome"
OUT="../../../resources/libraries/shapes.csh"
...
node -e '
  ...
  if (bytes.length < 1e6) {
    console.error("self-test failed: shapes.csh is suspiciously small (" + bytes.length + " bytes)");
    process.exit(1);
  }
' || { echo "ERROR: shapes.csh self-test failed" >&2; exit 1; }

echo "built shapes.csh"
